...
Game::Game():mWindow(sf::VideoMode(1500, 1500), "Ultimate Tic Tac Toe"), mBoard1(), mBoard2(), mBoard3(), mBoard4(), mBoard5(),
...
void Game::processEvents(){
    ...
    while (mWindow.pollEvent(event))
    {
        ...
        // was control releasedr
        if (event.type == sf::Event::KeyReleased && (event.key.code == sf::Keyboard::LControl || event.key.code == sf::Keyboard::RControl)){
            controlPressed = false;
        }
        
        if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::Z && !start){
            printf("undone bitches");
        }
...
void Game::drawO(int boxNum, int boardNum){
    // Create a graphical text to display
    sf::Font font;
    if (!font.loadFromFile(resourcePath() + "Playtime.ttf")) {
        return EXIT_FAILURE;
